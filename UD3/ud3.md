[Volver al índice general](../README.md)

# UD3 – Planificación y ejecución del proyecto técnico

## Índice de apartados

1. [Planificación de actividades](./documentos/1_Planificación_de_actividades.md)
2. [Gestión de recursos y logística](./documentos/2_Gestión_de_recursos_y_logística.md)
3. [Procedimientos de ejecución](./documentos/3_Procedimientos_de_ejecución.md)
4. [Gestión de riesgos y seguridad](./documentos/4_Gestión_de_riesgos_y_seguridad.md)
5. [Valoración económica y control de costes](./documentos/5_Valoración_económica_y_control_de_costes.md)

# Webgrafía – Documentación Oficial

- **Nmap** — https://nmap.org/book/man.html  
- **Suricata IDS** — https://docs.suricata.io/  
- **Emerging Threats Rules** — https://rules.emergingthreats.net/  
- **Grafana OSS** — https://grafana.com/docs/  
- **Ubuntu Server** — https://ubuntu.com/server/docs  
- **pfSense CE** — https://docs.netgate.com/pfsense/en/latest/  
- **VirtualBox** — https://www.virtualbox.org/manual/  
- **GitHub Docs** — https://docs.github.com/
