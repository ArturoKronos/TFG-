[Volver al índice general](../README.md)

# UD2 – Diseño técnico de la infraestructura empresarial segura y automatizada


## Índice de apartados

1. [Recopilación de información técnica](./documentos/1_Recopilación_de_información_tecnica.md)
2. [Diseño lógico y físico de la infraestructura](./documentos/2_Diseño_lógico_y_físico_de_la_infraestructura.md)
3. [Definición de objetivos y fases del proyecto](./documentos/3_Definición_de_objetivos_y_fases_del_proyecto.md)
4. [Recursos y presupuesto](./documentos/4_Recursos_y_presupuesto.md)
5. [Documentación técnica](./documentos/5_Documentación_técnica.md)



# Webgrafía – Documentación Oficial

Referencias principales para el proyecto de infraestructura segura y automatizada en Cocacusca S.L.

---

## **1. Suricata (IDS/IPS)**

- **Documentación oficial completa**  
  [https://docs.suricata.io/en/suricata-8.0.2/](https://docs.suricata.io/en/suricata-8.0.2/)

- **Guía oficial de instalación y configuración**  
  [https://suricata.io/documentation/](https://suricata.io/documentation/)

---

## **2. Nmap (análisis de red y auditoría)**

- **Manual oficial en español**  
  [https://nmap.org/man/es/index.html](https://nmap.org/man/es/index.html)

- **Referencia oficial de comandos**  
  [https://nmap.org/book/man.html](https://nmap.org/book/man.html)

---

## **3. Grafana (visualización y dashboards)**

- **Documentación oficial**  
  [https://grafana.com/docs/grafana/latest/](https://grafana.com/docs/grafana/latest/)

---

## **4. Linux (Ubuntu/Debian)**

- **Documentación oficial de Ubuntu Server**  
  [https://ubuntu.com/server/docs](https://ubuntu.com/server/docs)

- **Documentación oficial de Debian**  
  [https://www.debian.org/doc/manuals/debian-reference/](https://www.debian.org/doc/manuals/debian-reference/)

---

## **5. Seguridad y normativas**

- **INCIBE / CERTSI – Guía oficial IDS/IPS y SIEM**  
  [https://www.incibe.es/sites/default/files/contenidos/guias/doc/certsi_diseno_configuracion_ips_ids_siem_en_sci.pdf](https://www.incibe.es/sites/default/files/contenidos/guias/doc/certsi_diseno_configuracion_ips_ids_siem_en_sci.pdf)

- **ISO/IEC 27001 – Gestión de seguridad de la información**  
  [https://www.iso.org/isoiec-27001-information-security.html](https://www.iso.org/isoiec-27001-information-security.html)
